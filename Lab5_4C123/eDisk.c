...
// **write this function**
	...
	start_address = EDISK_ADDR_MIN + (512 * (uint32_t)sector);
	uint8_t* ptROM;
	ptROM = (uint8_t*)start_address;								// (uint8_t*) is a 32-bit address pointer to an 8-bit variable (1Byte)
	if (start_address <= EDISK_ADDR_MAX){
		for (int i=0; i<512; i++){
			*buff = *ptROM;
			buff++;
			ptROM++;
...
// **write this function**
  uint32_t start_address;
	...
	ptRAM = (uint32_t*)buff;
	start_address = EDISK_ADDR_MIN + (512 * (uint32_t)sector);
	int result_FlashWrite;
			
	if (start_address < EDISK_ADDR_MAX){
		result_FlashWrite = Flash_WriteArray(ptRAM, start_address, 128);		
		if (result_FlashWrite != 128){
			return RES_ERROR;
		}
	} else{
		return RES_PARERR;
...
// **write this function**
	uint32_t start_address;
	int result_FlashErase;
	start_address = EDISK_ADDR_MIN;
	while ((start_address < EDISK_ADDR_MAX)){
		result_FlashErase = Flash_Erase(start_address);
		if (result_FlashErase != 0){
			return RES_ERROR;
		}
		start_address = start_address + 0x400;    // 1024 = 0x400
	}
  return RES_OK;
}
